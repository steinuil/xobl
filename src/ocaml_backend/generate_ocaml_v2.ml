...
let t_field_type ~ctx ~loc { ft_type; ft_allowed } =
  ...
  | Some (Allowed_enum enum) -> t_module_ident ~suffix:"enum" ~ctx ~loc enum "t"
  | Some (Allowed_mask mask) -> t_module_ident ~suffix:"mask" ~ctx ~loc mask "t"
  ...
  | fields ->
      let fields =
        ListLabels.map fields ~f:(fun field ->
            let name =
              name_of_field field |> Option.get |> Ident.snake |> with_loc ~loc
            in
            let typ = t_visible_field ~ctx ~loc field in
...
let td_enum ~loc = function
  | Enum { name; items } ->
      td_type ~suffix:"enum" ~loc name (t_enum_items ~loc items) |> Option.some
  | _ -> None
...
let stri_mask ~loc = function
  | Mask { name; items; additional_values } ->
      let items =
        ListLabels.map items ~f:(fun (name, value) ->
            [%stri
              let [%p p_id ~loc name] : t = of_int32 (bit [%e e_int ~loc value])])
      in
      let values =
...
let str_variant_t ~ctx ~loc items =
  let items, types = List.map (rf_variant_item ~ctx ~loc) items |> List.split in
  ...
  | fields ->
      ...
      let init =
        let no_optional_fields =
          List.for_all (function Field_optional _ -> false | _ -> true) fields
        in
